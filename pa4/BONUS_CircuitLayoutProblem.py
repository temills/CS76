#Class for solving circuit layout csp, taking advantage of flipped solutions
#Tracey Mills, 10/21/21

from ConstraintSatisfactionProblem import ConstraintSatisfactionProblem

class BONUS_CircuitLayoutProblem():
    ...
    def __init__(self, board, components):
        ...
        #take biggest domain and reduce up to 1/4 size
        comp = 0
        for i in range(len(domain)):
            if len(domain[i]) > len(domain[comp]):
                comp = i
        #keep 1 loc for each rotation quadruple
        new_domain = []
        ignore = []
        for loc in domain[comp]:
            if loc in ignore:
                continue
            else:
                #new loc, add to domain
                new_domain.append(loc)
                y = int(loc/self.nCols)
                x = loc - (y*self.nCols)
                yFlipped = self.nRows-y-len(self.components[comp])
                xFlipped = self.nCols-x-len(self.components[comp][0])
                #ignore vertical, horizontal, and double rotation of this loc
                ignore.append(yFlipped*self.nCols + x)
                ignore.append(y*self.nCols + xFlipped)
                ignore.append(yFlipped*self.nCols + xFlipped)
        domain[comp] = new_domain
