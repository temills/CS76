...
class IDMinimaxAI():
    ...
    def choose_move(self, board):
        move = self.iterative_deepening_minimax(board)
    ...
    def iterative_deepening_minimax(self, board):
        ...
        for d in range(self.max_depth):
            m, v = self.minimax_decision(board, d)
            if (best_move_val == None or v > best_move_val):
                best_move = m
                best_move_val = v
            print("best move and val at depth " + str(d) + ":")
            print(str(best_move) + ", " + str(best_move_val))
        return best_move
